# Shared helper functions for the Tadawul (Saudi Exchange) signal tracker.
# Dot-sourced by Get-Signals.ps1 and Get-DailySummary.ps1 — not run directly.
# SAR-native: no USD conversion anywhere here, unlike the US version's TrackerLib.

$ErrorActionPreference = "Stop"
$ScriptsDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$RepoRoot   = Split-Path -Parent $ScriptsDir
$DataDir    = Join-Path $RepoRoot "data-sa"
if (-not (Test-Path $DataDir)) { New-Item -ItemType Directory -Path $DataDir | Out-Null }

function Read-Config {
    Get-Content (Join-Path $RepoRoot "config-sa.json") -Raw | ConvertFrom-Json
}
...
function Read-State {
    ...
    return [pscustomobject]@{
        ...
        warning_state  = [pscustomobject]@{}
        open_position  = $null
        portfolio      = [pscustomobject]@{
            balance_sar         = $cfg.starting_capital_sar
            realized_pl_sar     = 0
            purified_total_sar  = 0
        }
    }
}
...
function Write-Snapshot {
    ...
    $snapshot = [pscustomobject]@{
        ...
        portfolio        = [pscustomobject]@{
            balance_sar        = $State.portfolio.balance_sar
            realized_pl_sar    = $State.portfolio.realized_pl_sar
            purified_total_sar = $State.portfolio.purified_total_sar
        }
        config = [pscustomobject]@{
            watchlist        = $Cfg.watchlist
            watchlist_names  = $Cfg.watchlist_names
